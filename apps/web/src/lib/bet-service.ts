...
// ---------------------------------------------------------------------------
// Step 1 + 2 + 3 — prepare (no wallet signing yet)
// ---------------------------------------------------------------------------
export async function prepareBet(
  ...
  try {
    allBalances = await getAllUsdcBalances(walletAddress);
  } catch (err) {
  ...
  console.log(`[BetFlow]   Source       : ${bestChain.chainName} ($${bestChain.balance.toFixed(2)} available)`);
