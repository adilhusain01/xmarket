// LiFi bridge integration adapted for Chrome extension
...
// Helpers
// ---------------------------------------------------------------------------
function getUsdcAddress(chainId: number, isTestnet: boolean = false): string {
...
export async function getBridgeRoutes(
  ...
  const fromName =
    USDC_CHAINS.find((c) => c.chainId === fromChainId)?.name ?? `Chain ${fromChainId}`;
  ...
  const { routes } = await getRoutes({
    ...
    fromTokenAddress: getUsdcAddress(fromChainId, isTestnet),
    toTokenAddress: getUsdcAddress(TARGET_CHAIN_ID, isTestnet),
 ...
 * wallet, then execute the chosen route.
 *
 * @param route         The Route from getBridgeRoutes.
 * @param walletClient  viem WalletClient.
 * @param switchChainFn Function to switch chains if needed.
 */
